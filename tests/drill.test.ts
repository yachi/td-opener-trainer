...
describe('D1: createDrillState', () => {
  ...
  test('rejects unplayable bag: Honey Cup L before I with hold occupied', () => {
    // Honey Cup: L target needs I below it. If S is held and L arrives before I → stuck
    // Bag: S, L, ... (hold S first, then L arrives but can't place or hold)
    // This bag should NOT be generated by createDrillState
    const bag: PieceType[] = ['S', 'L', 'O', 'J', 'Z', 'T', 'I'];
    const state = createDrillStateWithBag('honey_cup', bag, false);
    // createDrillStateWithBag doesn't validate — it's for testing.
    // But createDrillState should never produce this pattern.
    // Verify over 100 generations that L never arrives before I when hold is occupied.
    for (let i = 0; i < 100; i++) {
      const s = createDrillState('honey_cup');
      const seq = getOpenerSequence('honey_cup', s.mirror);
      const holdPiece = seq.holdPiece;

      // Find index of hold piece and I piece in bag
      const holdIdx = s.bagPieces.indexOf(holdPiece);
      const iIdx = s.bagPieces.indexOf('I' as PieceType);
      const lIdx = s.bagPieces.indexOf('L' as PieceType);

      // If L is the hold piece (normal side), it shouldn't matter
      // But if L is NOT the hold piece and arrives before I,
      // the bag should still be playable (hold can save it)
      // The key test: no bag should cause a stuck state
