...
- `src/session.ts` — unified reducer, `SessionAction` union (20 actions including intent actions `primary`/`pick`, browse action `browseOpener`, `selectPcSolution`, `selectDpcSolution`, and `jumpToBag`), `PHASE_META` table (exported, `satisfies Record<Phase, PhaseMeta>` for compile-time completeness, 9 phases including guess4/reveal4/reveal5), `isRevealPhase`/`isGuessPhase` helpers, `RevealSnapshot` type, `assertSessionInvariants` (10+ runtime invariants including 9b stale-snapshot detection, 3c dpcSolutionIndex bounds, and 3d reveal5 bag5 PC validity), `InvariantViolation`, `createSession`, `createDpcSession(holdPiece)` (DPC-direct entry — starts at guess4 with null guess, `dpcHoldPiece` set). `isDpcDirectSession(s)` predicate. `getDpcSolutionsForSession(s)` reads `dpcHoldPiece` (1-line lookup, no PC chain). `deriveBoard` centrally computes board from `(baseBoard, cachedSteps, step)` for reveal+auto states. `deriveSnapshots(prev, next, action)` centrally manages snapshot save/clear — individual actions NEVER set `revealSnapshots:` (dependency DAG: reveal1←opener, reveal2←route, reveal3←pc, reveal4←dpcSolutionIndex, reveal5←dpcSolutionIndex; jumpToBag detected as restore, skips derivation). `hardDrop` uses `cachedSteps[step].board` (engine-validated) instead of raw `stampCells`; auto-advance restricted to `reveal2` only. `jumpToBag` restores from `revealSnapshots` in O(1), supports bags 1-5. **Does NOT import stampCells/lockAndClear** — all boards come from engine-validated functions. Production `sessionReducer` wraps raw reducer with deriveBoard → deriveSnapshots → invariant check. DPC-direct sessions: `newSession`/`advancePhase` loop back to guess4 (via `restartDpcDirect`), invariants 5 and 3c relaxed for null guess with dpcHoldPiece.
- `src/renderer/session.ts` — single phase-aware renderer, reads everything from Session (no static doctrinal reads except rule card + opener name). `drawReveal1Panel` shows user's held piece in manual mode (Bug #1 fix). 9 phase panels: guess1/reveal1/guess2/reveal2/guess3/reveal3/guess4/reveal4/reveal5. Nav bar with 5 contextual breadcrumb tabs (HC ✓, Rt 3/8, PC 2/4, DPC 1/6, PC —). `drawGuess4Panel` lists DPC solutions for selection; shows "DPC Practice (Hold: O)" label in DPC-direct mode. `drawReveal4Panel` shows DPC build step-by-step with hold piece and "DPC Complete!" banner. `renderWelcome(ctx, now, dpcDirect)` draws the pre-session welcome screen (title, flow steps, keybinds, D key DPC toggle, pulsing CTA).
...
**Tests (28 files, 1652 tests, ~8.6K assertions, ~39s full suite):**
- `tests/guard-matrix.test.ts` — 381 tests, declarative guard matrix (20 actions × 16 contexts) + edge cases + phase metadata structural tests. Compile-time completeness: adding a new action without guard spec OR a new phase without PHASE_META entry is a type error.
