...
### #21. Centralize derived state — don't scatter it across action cases
Snapshot management was scattered across 4 action cases (submitGuess, selectRoute, selectPcSolution, browseOpener), each manually saving/clearing `revealSnapshots`. This caused 2 staleness bugs (browseOpener didn't clear downstream, selectRoute didn't clear reveal3) and made the system fragile to new actions — same disease as scattered phase-string guards (lesson #18).

**The L9 reframe**: snapshot consistency is DERIVED STATE, not an obligation of each action. Same pattern as `deriveBoard` — move ALL snapshot logic to ONE post-reduction step: `deriveSnapshots(prev, next, action)`. The dependency DAG (reveal1←opener, reveal2←route, reveal3←pc) determines CLEAR rules. SAVE fires when entering a reveal phase with new cachedSteps. jumpToBag is detected as a restore (not a decision) and skips derivation.

**Rule**: when a post-reduction property depends on comparing prev vs next state (cache invalidation, derived indexes, undo history), centralize it in the reducer wrapper pipeline — never scatter it across N action cases. The pipeline is: `_rawSessionReducer` → `deriveBoard` → `deriveSnapshots` → `assertSessionInvariants`. Each step handles one concern. Adding a new action that changes opener/route/PC gets correct snapshot behavior for free. Commit `a68121c`.
...
### Snapshot management goes through deriveSnapshots — never in action cases
Individual reducer action cases MUST NOT set `revealSnapshots:`. All snapshot save/clear logic lives in `deriveSnapshots(prev, next, action)`, called in the post-reduction pipeline. The dependency DAG: `reveal1 ← (opener, mirror)`, `reveal2 ← (routeGuess)`, `reveal3 ← (pcSolutionIndex)`, `reveal4 ← (dpcSolutionIndex)`, `reveal5 ← (dpcSolutionIndex)`. Upstream changes auto-clear downstream snapshots. `jumpToBag` is a restore (not a decision) and skips derivation.

**Enforced by**: `tests/diag-l9-snapshot-centralize.test.ts` — equivalence proof (§1), consistency invariant (§2), SNAPSHOT_DEPS structural (§3), invariant 9b load-bearing (§4). Mutation testing: 5/5 guards load-bearing.
...
- `src/session.ts` — unified reducer, `SessionAction` union (22 actions including intent actions `primary`/`pick`, browse action `browseOpener`, `selectPcSolution`, `selectDpcSolution`, `selectBag5PcSolution`, `resetDpcHold`, and `jumpToBag`), `PHASE_META` table (exported, `satisfies Record<Phase, PhaseMeta>` for compile-time completeness, 9 phases including guess4/reveal4/reveal5), `isRevealPhase`/`isGuessPhase` helpers, `DPC_HOLD_PIECES` (valid hold pieces: O,S,Z,I,J,L), `RevealSnapshot` type, `assertSessionInvariants` (10+ runtime invariants including 9b stale-snapshot detection, 3c dpcSolutionIndex bounds, and 3d reveal5 bag5 PC validity), `InvariantViolation`, `createSession`, `createDpcSession(holdPiece | null)` (DPC-direct entry — null = hold picker sub-state, PieceType = focused practice). `isDpcDirectSession(s)` predicate (covers both null-hold picker and set-hold practice). `getDpcSolutionsForSession(s)` reads `dpcHoldPiece` (1-line lookup, no PC chain). `deriveBoard` centrally computes board from `(baseBoard, cachedSteps, step)` for reveal+auto states. `deriveSnapshots(prev, next, action)` centrally manages snapshot save/clear — individual actions NEVER set `revealSnapshots:` (dependency DAG: reveal1←opener, reveal2←route, reveal3←pc, reveal4←dpcSolutionIndex, reveal5←(bag5PcSolutionIndex, dpcSolutionIndex); jumpToBag detected as restore, skips derivation). `hardDrop` uses `cachedSteps[step].board` (engine-validated) instead of raw `stampCells`; auto-advance restricted to `reveal2` only. `jumpToBag` restores from `revealSnapshots` in O(1), supports bags 1-5. **Does NOT import stampCells/lockAndClear** — all boards come from engine-validated functions. Production `sessionReducer` wraps raw reducer with deriveBoard → deriveSnapshots → invariant check. DPC-direct sessions: `newSession`/`advancePhase` loop back to guess4 (via `restartDpcDirect`), invariants 5 and 3c relaxed for null guess with dpcHoldPiece. `pick` in guess4 with null hold sets `dpcHoldPiece` from `DPC_HOLD_PIECES`; with set hold selects DPC solution. `resetDpcHold` returns to hold picker (null hold). D key dispatches `resetDpcHold`.
- `src/renderer/session.ts` — single phase-aware renderer, reads everything from Session (no static doctrinal reads except rule card + opener name). `drawReveal1Panel` shows user's held piece in manual mode (Bug #1 fix). 9 phase panels: guess1/reveal1/guess2/reveal2/guess3/reveal3/guess4/reveal4/reveal5. Nav bar with 5 contextual breadcrumb tabs (HC ✓, Rt 3/8, PC 2/4, DPC 1/6, PC 1/3). `drawGuess4Panel`: hold picker sub-state shows 6 hold pieces (O,S,Z,I,J,L) with "1-6 pick"; set-hold sub-state lists DPC solutions with "D change hold" hint. `drawReveal4Panel` shows DPC build step-by-step with hold piece and "DPC Complete!" banner. `renderWelcome(ctx, now, dpcDirect)` draws the pre-session welcome screen (title, flow steps, keybinds, D key DPC toggle, pulsing CTA).
- `src/renderer/board.ts` — canvas primitives, `COLORS` palette, `drawCell`, `drawPieceInBox`
- `src/input/keyboard.ts` — dumb key→intent mapper. DAS/ARR timing lives here. Uses `isRevealPhase()` and `PHASE_META` from session.ts — no hardcoded phase strings. No phase-specific branching for SPACE/ENTER/digits (those dispatch intents; reducer interprets). `[`/`]` dispatch `jumpToBag`. D key dispatches `resetDpcHold`.
- `src/openers/bag3-pc.ts` — Perfect Clear solutions for all 4 openers (route-keyed), `getPcSolutions(opener, mirror, routeIndex)`. HC: 12 solutions (10 routes). GM: 7 solutions (routes 0-3; route 4 no PC). MS2: 11 solutions (5 routes; route 4 bonus has 7-piece solutions with duplicate I). SC: 3 solutions (routes 0,3; routes 1,2,4 no PC).
- `src/openers/bag4-dpc.ts` — DPC solutions for bags 4-5, `getDpcSolutions(holdPiece)`. Source: Hard Drop wiki DPC_Setups. 8 named setups + mirrors = 16 total. Hold coverage: O(4), S(2), Z(2), I(6), J(1), L(1), T(0 — OSZ Ball requires 180° rotation, won't-fix). Normals stored for O/S/I/J; Z=mirror(S), L=mirror(J); O/I include self-mirrors. 14 TSD (clear 2 lines, 12 remaining), 2 TSS (J/L-hold TSM J SPC). All BFS-validated via buildSteps + replayPcSteps.
- `src/openers/bag5-pc.ts` — Bag 5 PC solutions completing the DPC cycle, `getBag5PcSolution(holdPiece, dpcSolutionIndex)` (first solution) and `getBag5PcSolutions(...)` (all). All 16 DPC variants have solutions (30 total: O 2+3+2+3, S 2+2, Z 2+2, I 1+2+2+1+2+2, J 1, L 1). Shared boards: O-Kuruma = S-Kuruma, O-Kuruma Mirror = Z-Kuruma Mirror. All 7 pieces placed, BFS-validated via replayPcSteps. I#0/I#3 Fake Butter use intermediate line clears; all others are static tilings. TSM J SPC (J/L-hold) use 5-row PC (22-cell post-TSS boards). Multi-solution selection: `bag5PcSolutionIndex` on Session, digit keys in reveal5 auto mode switch solutions (1-3 per variant). Nav bar shows `PC 1/3`, panel shows `Solution 1/3`.
...
- `tests/fixtures/drill-steps-golden.json` — precomputed `buildSteps` output for all 50 opener×mirror×route combos (28KB). Source of truth = placement data, NOT code output. If code diverges, investigate — don't regenerate.
- Scripts: `test:fast` (19 files, ~15s dev loop), `test:slow` (2 heavy files), `test:ci` (CI=true, full PBT ~60s)

**Tests (29 files, 1869 tests, ~9.3K assertions, ~44s full suite):**
- `tests/guard-matrix.test.ts` — 417 tests, declarative guard matrix (22 actions × 16 contexts) + edge cases + phase metadata structural tests. Compile-time completeness: adding a new action without guard spec OR a new phase without PHASE_META entry is a type error.
...
- `tests/diag-l9-property.test.ts` — 13 fast-check properties, dev=50 runs (via pbt-config.ts), CI=2000 runs. Covers all 20 action types (including selectPcSolution, selectDpcSolution + jumpToBag) + float/NaN/Infinity rejection.
...
- `tests/diag-l9-pc-routes.test.ts` — 300 tests, route-specific PC proof for all 4 openers: solution counts match expected (§1), every solution achieves Perfect Clear (§2), every placement BFS-reachable (§3), structural invariants including 7-piece MS2 Bonus (§4)
...
- `tests/diag-l9-bag5-pc.test.ts` — 129 tests, bag 5 PC proof: solution existence all 16 variants (§1), solution counts 30 total (§2), Perfect Clear achieved (§3), BFS reachability (§4), all 7 pieces used (§5), shared board references (§6)
- `tests/diag-l9-dpc-hold-picker.test.ts` — 30 tests, DPC hold picker proof: createDpcSession(null) (§1), isDpcDirectSession widened (§2), pick sets hold (§3), resetDpcHold (§4), full lifecycle (§5), invariants (§6), edge cases (§7)
...
- `tests/diag-drill-queue.test.ts` — 175 tests, drill-queue ordering. Diag 4 uses golden fixture (was 131s×2 DFS exhaustion, now <1s fixture read).
